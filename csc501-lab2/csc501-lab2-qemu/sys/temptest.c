...
void reader3 (char *msg, int lck, int hint)
{
	...
	//	}
	//}//i
        kprintf ("  %s: acquired lock PRIOIRTY %d\n", msg, proctab[currpid].pinh);
        kprintf ("  %s: to release lock\n", msg);
        releaseall (1, lck);
}
...
void test3 ()
{
	...
	int rd4, rd5, wr2;	
	int s = screate(1);
        rd4 = create(reader4, 2000, 20, "reader3", 2, "A", s);
        rd5 = create(reader4, 2000, 50, "reader3", 2, "B", s);
        wr2 = create(writer4, 2000, 25, "writer3", 2, "C", s);
